...
- [第三节 矿车动力学](#第三节-矿车动力学)
	...
	- [水平动力直轨上的运动规律](#水平动力直轨上的运动规律)
	- [货运矿车单格最大滑行距离的推导](#货运矿车单格最大滑行距离的推导)
	- [多动力铁轨加速阻尼实验](#多动力铁轨加速阻尼实验)
	- [隐速度阈值](#隐速度阈值)
	- [多格加速模型](#多格加速模型)
		- [加速过程](#加速过程)
		- [匀速加动量过程](#匀速加动量过程)
...
![](https://github.com/Xiaoyuan-xyz/The-Principle-of-Redstone-Circuits/raw/master/Illustration/铁轨及矿车系统/铁轨及矿车系统-8a4fb8d6.png)
...
为了研究动力铁轨如何对矿车进行加速，玩家们设计了如下的对照实验：只使用1格动力铁轨作为加速，记录不同矿车在阻尼直轨上的最远运行距离（下称最大滑行距离$S_\mathbb{max}$）。这里阻尼直轨指的是普通铁轨、探测铁轨或激活铁轨，因为实验证明矿车在这三种铁轨上的减速过程完全相同。
...
- 在单格动力铁轨加速中，除动力矿车外，其他矿车均未到达饱和速度。饱和速度指的是游戏为每类矿车设定的最大速度值，动力矿车的饱和速度是0.2m/gt，其他矿车的饱和速度是0.4m/gt（=8m/s）。
...
![](https://github.com/Xiaoyuan-xyz/The-Principle-of-Redstone-Circuits/raw/master/Illustration/铁轨及矿车系统/铁轨及矿车系统-c9168b8a.png)

经过足够长的时间（>3000gt），可以认为所有矿车静止，从而测出矿车的最大滑行距离。

![](https://github.com/Xiaoyuan-xyz/The-Principle-of-Redstone-Circuits/raw/master/Illustration/铁轨及矿车系统/铁轨及矿车系统-4b22127f.png)
...
![](https://github.com/Xiaoyuan-xyz/The-Principle-of-Redstone-Circuits/raw/master/Illustration/铁轨及矿车系统/铁轨及矿车系统-b916a381.png)
...
![](https://github.com/Xiaoyuan-xyz/The-Principle-of-Redstone-Circuits/raw/master/Illustration/铁轨及矿车系统/铁轨及矿车系统-f205234b.png)
...
- 矿车的真实速度由相邻两游戏刻间的位移差来刻画，用$v$或来表示，第$t$和$t+1$游戏刻的位移差用$v_t$来表示，也就有$v_t=x_{t+1}-x_t$。
- 矿车的Motion似乎从某种程度上反映了矿车的真实速度，是一个与矿车速度有关的量，以下称为运动量或隐速度，这里用$M$表示Motion。
- 矿车在相邻两游戏刻间的Motion增量从某种程度上反映了矿车的加速度，是一个与矿车加速有关的量，以下称为加速度或加动量，这里用$a$表示，即$a_t=M_{t+1}-M_t$。
...
- 矿车存在饱和速度$v_\mathbb{max}$，对动力矿车来说，这个值是0.2，对其他矿车来说，这个值是0.4。因而动力矿车在所有矿车中移动得最慢。
...
- 对非运矿车、货运矿车和动力矿车来说，在达到饱和速度前，$v=M$，而客运矿车却有$v=0.75M$。这里我们把系数0.75称为质量因子，记作$m$。对客运矿车来说有$m=0.75$，对其他矿车来说有$m=1$，可以得出：

$$v=\mathbb{max}(v_\mathbb{max},mM)$$

- 在第一游戏刻，所有矿车的隐速度均为0.02，可以认为这是位于实体方块毗邻的矿车被动力铁轨给予的初始隐速度，即$M_1=0.02$。
- 矿车在第一游戏刻的加速度接近但略小于0.06，此后的加速度随速度增加而递减，可以认为动力铁轨中仍然存在阻尼作用，且有：

$$M_{t+1}=dM_t+a$$

这里$a=0.06$，表示动力铁轨的加速作用，$d$称为阻尼乘数，其试图用离散方式模拟与速度成正比的阻力加速度带来的减速效应。
...
以此预期，矿车的隐速度也会存在饱和现象，即在足够长（大于400格）的动力铁轨加速下，可以认为动力铁轨的加速作用和动力铁轨的阻尼减速作用带来的结果相同。此时称这个速度为饱和隐速度或终端速度，或称为饱和运动量，记为$M_\mathbb{max}$。应当有$M_\mathbb{max}=dM_\mathbb{max}+a$，即
...
![](https://github.com/Xiaoyuan-xyz/The-Principle-of-Redstone-Circuits/raw/master/Illustration/铁轨及矿车系统/铁轨及矿车系统-3c15168d.png)

![](https://github.com/Xiaoyuan-xyz/The-Principle-of-Redstone-Circuits/raw/master/Illustration/铁轨及矿车系统/铁轨及矿车系统-874daef3.png)
...
![](https://github.com/Xiaoyuan-xyz/The-Principle-of-Redstone-Circuits/raw/master/Illustration/铁轨及矿车系统/markdown-img-paste-20200127184323542.png)
...
可以看出，阻尼乘数越大，达到隐速度越快，需要的动力铁轨数越少。非运矿车和动力矿车的终端速度小于2，这两种矿车不可能通过动力铁轨加速达到隐速度阈值，故只考虑客运矿车和货运矿车的隐速度阈值。
...
下面假设动力铁轨的数量未能使矿车达到隐速度阈值，再将整个多格动力铁轨加速阻尼的全过程分为加速过程、匀速加动量过程、匀速减动量过程和减速过程四个过程。
...
| $t_A$              | 9        | 10             | 8                | 8                | 5                |
| $v_{t_A}(M_{t_A})$ | 0.4323   | 0.4148(0.5530) | 0.4330           | 0.4130           | 0.2352           |
...
在匀速加动量过程$t_A\le t<t_B$中，矿车仍然处于动力铁轨之上，但已到达饱和速度，此时矿车仅增加隐速度。由于铁轨总是整格的，假设在这一过程中矿车经过了$n$格动力铁轨，那么动力矿车会花费 $\Delta t=5n$的时间，而其他矿车因为存在奇偶效应，需要花费的时间为
$\Delta t=\left\{
             \begin{array}{**ll**}
...
其中 $x_{t_B}$ 是一个略大于$n-0.5$的数（因为其恰刚离开第$n$格铁轨），这里统一将其写为$x_{t_B}=n-(1-\sigma)$ 的形式， $\sigma$称为位移偏置常量，仅与矿车类型与动力铁轨奇偶性有关。具体有 $\sigma=x-\lfloor x\rfloor$，$x$在这里指离开动力铁轨时的位移，特别地，$\sigma=x_{t_A}-\lfloor x_{t_A}\rfloor$。
...
在匀速减动量过程$t_B\le t<t_C$中，矿车进入阻尼直轨，此时加速效果消失，但因矿车隐速度较大，其保持匀速，也就有$mM>v_\mathbb{max}$。
...
减速过程$t_C\le t<+\infty$是最后一个过程，随着矿车的隐速度不断下降，矿车将以$v_{t_C}$ 为初速做阻尼减速运动，这种运动在之前已经进行过讨论，在这里我们知道
$$\begin{align}
...
有时候，我们不需要精准预测出矿车的位置，只要大致知道最大滑行距离即可，为此我们可以估计$S_\mathbb{max}$。在上式中，我们认为$v_{t_C}=v_\mathbb{max}$，这就有：
[^_^]:
$$\begin{align}
\hat{S}_\mathbb{max}
&=v_\mathbb{max} \log_d\dfrac{v_\mathbb{max}}{mM_{t_B}}+\dfrac{v_\mathbb{max}}{1-d}+\sigma\\
&=v_\mathbb{max}(\log_d\dfrac{v_\mathbb{max}}{m((M_1-M_\mathbb{max})d^{t_B-1}+M_\mathbb{max})}+\dfrac{1}{1-d})+\sigma\\
&=v_\mathbb{max}(\log_dv_\mathbb{max}-\log_dm-\log_d((M_1-\dfrac{a}{1-d})d^{t_B-1}+\dfrac{a}{1-d})+\dfrac{1}{1-d})+\sigma\\
&\approx v_\mathbb{max}(\log_d\dfrac{(1-d)v_\mathbb{max}}{am}-\log_d(1-d^{t_B-1})+\dfrac{1}{1-d})+\sigma\\
\end{align}$$
...
下面要做的就是解释隐速度阈值的具体实现过程，以及在模型中加入隐速度阈值。

## 货运矿车的阈值加速模型

货运矿车和客运矿车在达到隐速度阈值后的最大滑行距离称为阈值滑行距离，其仅与矿车类型与动力铁轨奇偶性有关，记为$S_\mathrm{TH}$，计算方式在上一节中已经讨论过了，这里只写下结论
